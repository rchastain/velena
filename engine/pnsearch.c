...
void change_sequence()
{
  ...
  scanf("%hd%hd%hd%hd%hd%hd%hd", &seq[0], &seq[1], &seq[2], &seq[3], &seq[4], &seq[5], &seq[6]);
...
void evaluate(struct node *node)
{
  ...
  */
  if (auxboard->filled >= MAXMEN) fatal_error("I thought I never would have come here...");

  oracle = think_function(auxboard); // RC
...
short start_pn_search(struct board *board)
{
  ...
  */
  ...
  printf("Now building nodes... (each node takes %ld bytes of memory)\n",
         sizeof(struct node));
